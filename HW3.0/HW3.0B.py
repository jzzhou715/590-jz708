...
GAMMA_L1	=	0.00	#L1 REGULARIZATION CONSTANT
GAMMA_L2	=	0.01	#L2 REGULARIZATION CONSTANT
alpha		=	0.25	#MOMENTUM PARAMETER
#ANN PARAM
layers		=	[2,3,2,1] #FIRST NUMBER OVERWRITTEN LATER BY INPUT SHAPE
activation	=	"SIGMOID"	#SIGMOID OR TAHN
IPLOT=True
I_NORMALIZE=True;
PARADIGM='mini_batch'
dx=0.0001			#STEP SIZE FOR FINITE DIFFERENCE
...
#         y = S(np.matmul(x, submatrices[0].transpose()) + submatrices[1].transpose())
#         y = S(np.matmul(y, submatrices[2].transpose()) + submatrices[3].transpose())
#         y = np.matmul(y, submatrices[4].transpose()) + submatrices[5].transpose()
...
def NN_eval(x, submatrices):
    if activation == "SIGMOID":
        y = S(np.matmul(x, submatrices[0].transpose()) + submatrices[1].transpose())
        for i in range(2,len(submatrices)-2,2):
            y = S(np.matmul(y, submatrices[i].transpose()) + submatrices[i+1].transpose())
        y = np.matmul(y, submatrices[len(submatrices)-2].transpose()) + submatrices[len(submatrices)-1].transpose()
...
def loss(p,index_2_use):
	...
	if GAMMA_L1 != 0 and GAMMA_L2 != 0:
            raise ValueError('Please specify ONLY ONE gamma for regularization')
	...
	for i in p[0::2]:
            L2 += np.sum(i**2)
	L2 = GAMMA_L2*L2
	training_loss=np.mean(errors**2.0)+L1+L2			#MSE
