...
#MINIMIZER FUNCTION
#------------------------
def minimizer(f,xi, algo='MOM', LR=0.01, EDF = 0.01):
	...
	while(iteration<=max_iter):
		...
		if(PARADIGM == 'batch'):
			...
			if(iteration==1):
                                ...
                                if(index_2_use == len(train_idx)):
                                   index_2_use = 0
                                else:
                                   index_2_use += 1
		elif(PARADIGM == 'minibatch'):
                        mid_ind = len(train_idx)//2
                        batch1 = train_idx[:mid_ind]
                        batch2 = train_idx[mid_ind:]
                        if(iteration%2==0):
                                index_2_use = batch1
                        else:
                                index_2_use = batch2
		...
		for i in range(0,NDIM):	#LOOP OVER DIMENSIONS
			...
			delta = grad_i - df_dx[i-1]
			df_dx[i]=grad_i 
			
		#TAKE A OPTIMIZER STEP
		if(algo=="GD"):  xip1=xi-LR*df_dx 
		if(algo=="MOM"): xip1=xi-LR*df_dx-EDF*delta
